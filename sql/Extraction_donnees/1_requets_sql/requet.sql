-- Active: 1693924903525@@127.0.0.1@3306@exemple
...
SELECT nom, salaire, nodep AS "numéro de département" 
FROM employe
...
« Secrétaire ».*/
SELECT nom, titre, nodep AS "numéro de département" , salaire 
FROM employe 
...
SELECT nom, titre, nodep as "numéro de département", salaire FROM employe 
WHERE titre in ( "Représentant","Secrétaire") AND nodep=34;
...
SELECT nom, salaire, tauxcom as "taux de commission"
FROM employe
ORDER BY tauxcom;
...
SELECT nom, salaire, tauxcom as "taux de commission", titre
FROM employe
...
SELECT nom, salaire, tauxcom as "taux de commission", (salaire * tauxcom/100)  as commission
FROM employe
WHERE tauxcom IS NOT NULL;
...
les colonnes*/
SELECT CONCAT (nom,prenom ) as "nom et prenom concaténé"
FROM employe;
...
employés.*/
SELECT nom, INSTR(nom, "r") AS "le rang de la lettre « r »"
FROM employe;

/*31. Afficher le nom, le nom en majuscule et le nom en minuscule de 
l'employé dont le nom est Vrante.*/
SELECT nom, UCASE(nom) AS "nom en majuscule", LCASE(nom) AS "nom en miniscule"
FROM employe
WHERE nom="Vrante";
